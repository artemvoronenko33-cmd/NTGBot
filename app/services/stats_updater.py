...
async def refresh_stats(db: AsyncSession, notify_admin: bool = True):
    """Полное обновление статистики: кэш, ежедневная, по товарам + уведомления об аномалиях"""
    today = date.today()
    ...
    async with db.begin():  # Главная транзакция
        try:
            ...
            return {"status": "error", "anomalies": anomalies}
